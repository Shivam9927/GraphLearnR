...
build_partial_ground_truth <- function(dist_filename = "NOAA_dist_increased.csv", data_path = getwd()) {
...
# -----------------------------------------------------------------------------
# Python-matched optimization steps
# -----------------------------------------------------------------------------
optimize_L <- function(hatS, P, beta, w1, w2, solver = "SCS") {
  ...
  objective <- Minimize(w1 * linear_term + w2 * quad_term)
...
optimize_hats <- function(L, P, beta, S, w1) {
  ...
  A <- diag(N * n) + w1 * kronecker(diag(n), L)
  b <- vecF(S) + w1 * (kronecker(diag(n), L) %*% Pbeta(P, beta))
...
GSPRegression <- function(X_noisy, P, w1, w2, max_iter = 100,
  ...
  for (i in seq_len(max_iter)) {
    raw_L <- optimize_L(hats, P, beta, w1, w2, solver = solver)
    hats <- optimize_hats(raw_L, P, beta, X_noisy, w1)
    beta <- optimize_beta(P, raw_L, hats)

    x <- vecF(hats) - Pbeta(P, beta)
    D <- kronecker(diag(n), raw_L)
    objective[i] <- norm(hats - hats0, type = "F")^2 +
      w1 * as.numeric(t(x) %*% D %*% x) +
      w2 * norm(raw_L, type = "F")^2
  ...
  list(
    L_pruned = prune(raw_L, threshhold),
    hatS = hats,
    beta = beta,
    raw_L = raw_L,
    objective = objective
...
run_one_noaa <- function(filename, data_path = getwd(), L_prime,
                         w1 = 0.001, w2 = 0.99,
                         max_iter = 100, prune_threshold = 1e-5,
                         convergence_tol = 1e-4, solver = "SCS") {
...
run_all_noaa <- function(data_path = getwd(),
  ...
  list(summary = summary_df, pooled = pooled, L_prime = L_prime)
}
