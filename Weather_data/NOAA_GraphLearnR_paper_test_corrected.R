...
# =============================================================================
# NOAA paper-aligned test script using the GraphLearnR package
# Corrected to use:
# - fit$L for evaluated/pruned Laplacian
# - fit$L_unpruned for raw-edge diagnostics
# - no extra pruning outside the package
# =============================================================================
...
run_one_setting <- function(filename, data_path, L_prime,
  ...
  if (is.null(fit$L)) stop("GraphLearnR::gsp_regression did not return fit$L")
  if (is.null(fit$L_unpruned)) stop("GraphLearnR::gsp_regression did not return fit$L_unpruned")

  metrics <- metricsprf_partial(L_prime, fit$L)
  ...
  data.frame(
    ...
    FN = metrics$fn,
    TN = metrics$tn,
    Learned_Edges = sum(lowertrian(fit$L) != 0),
    Raw_Edges = sum(lowertrian(fit$L_unpruned) != 0),
